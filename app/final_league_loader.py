...
"""
Sistema Final de Carregamento das Ligas Principais
- Usa endpoints válidos da API Sportmonks v3
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('final_league_loader.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class FinalLeagueLoader:
    def __init__(self):
    ...
    def get_season_id(self, year, league_id):
        ...
        try:
            ...
            logger.info(f"      📡 Buscando temporada {year} para liga {league_id}...")
    ...
    def load_fixtures_for_season(self, season_id, league_id, league_name, year):
        """Carregar todos os fixtures de uma temporada usando endpoint válido"""
        logger.info(f"   📅 Carregando temporada {year} da {league_name}...")
        
        try:
            fixtures_loaded = 0
            
            # Buscar em múltiplas janelas de 100 dias (limite da API)
            end_date = datetime.now()
            start_date = end_date - timedelta(days=100)
            
            # Buscar até 1 ano atrás em janelas de 100 dias
            max_lookback = 365
            current_lookback = 0
            
            while current_lookback < max_lookback:
                ...
                logger.info(f"      📅 Buscando janela: {start_date.strftime('%Y-%m-%d')} até {end_date.strftime('%Y-%m-%d')}")
                ...
                while True:
                    ...
                    # Filtrar fixtures da temporada e liga específicas
                    season_fixtures = []
                    for fixture in fixtures:
                        if (fixture.get("season_id") == season_id and 
                            fixture.get("league_id") == league_id):
                            season_fixtures.append(fixture)
                    
                    # Processar fixtures filtrados
                ...
                # Mover para a próxima janela de 100 dias
                end_date = start_date
                start_date = end_date - timedelta(days=100)
                current_lookback += 100
                
                time.sleep(2)  # Delay entre janelas
