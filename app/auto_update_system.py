...
- Sincronização inteligente com API Sportmonks
...
class AutoUpdateSystem:
    def __init__(self):
        ...
        # IDs das ligas para monitorar
        self.leagues_to_monitor = {
            648: "Série A Brasil",
            649: "Série B Brasil", 
            650: "Copa do Brasil",
            651: "Primeira Divisão Argentina",
            652: "Sudamericana",
            653: "Libertadores"
        }
        
        # Temporadas para monitorar
        self.seasons_to_monitor = [2023, 2024, 2025]
    ...
    def check_new_fixtures(self):
        ...
        try:
            ...
            # Buscar último fixture atualizado (usando starting_at como referência)
            cur.execute("SELECT MAX(starting_at) FROM fixtures")
            last_update = cur.fetchone()[0]
            
            if not last_update:
                last_update = datetime.now() - timedelta(days=7)  # Última semana
            else:
            ...
            for league_id, league_name in self.leagues_to_monitor.items():
                logger.info(f"   Verificando {league_name}...")
                
                url = f"{self.api_base}/fixtures/latest"
                params = {
    ...
    def generate_update_report(self, new_fixtures, updated_count):
        """Gerar relatório de atualização"""
        logger.info("📊 Gerando relatório de atualização...")
        
        report = {
            "timestamp": datetime.now().isoformat(),
            "new_fixtures": len(new_fixtures),
            "updated_count": updated_count,
            "leagues_checked": list(self.leagues_to_monitor.keys()),
            "seasons_checked": self.seasons_to_monitor
        }
        ...
        logger.info(f"   ✅ Relatório salvo: {report}")
        return report
    ...
    def run_update_cycle(self):
        ...
        try:
            ...
            # 4. Gerar relatório
            self.generate_update_report(new_fixtures, updated_count)
    ...
    def run_continuous(self):
        ...
        logger.info("🔄 INICIANDO SISTEMA CONTÍNUO DE ATUALIZAÇÃO")
        logger.info(f"   ⏰ Intervalo: {self.update_interval}s ({self.update_interval/3600:.1f}h)")
        logger.info(f"   🏆 Ligas monitoradas: {len(self.leagues_to_monitor)}")
        logger.info(f"   📅 Temporadas: {', '.join(map(str, self.seasons_to_monitor))}")
        logger.info("=" * 60)
        ...
        try:
            while True:
                ...
                logger.info(f"⏳ Próxima atualização em {self.update_interval/3600:.1f} horas...")
